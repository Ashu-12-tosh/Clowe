...
`start` scripts are deliberately bare (`next start`, `node dist/index.js`) so
whatever runs them decides the port: the production image sets `PORT`, and a
bare script obeys it. A hardcoded `-p` would silently override that and the
container would listen somewhere nginx is not looking.

If those ports are taken on your machine, put the overrides in `.env.local`
(gitignored, loaded by both apps and winning over `.env`):
...
The web dev server is the one exception: Next resolves its port before reading
env files, so `dev` carries `-p` directly. That is safe because `next dev`
is never containerised. To run a production build locally on the same port,
use `npm run start:local` instead of `npm start`.
...
5. Every run is kept: flip between results for this product in the panel, rate the fit, or see them all at http://localhost:3000/tryon
